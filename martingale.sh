...
show_complete_book_data() {
    ...
    local json_file="$SCRIPT_DIR/generer_book_${isbn}_complete.json"
    cat > "$json_file" << EOF
{
    "isbn": "$isbn",
    "identifiers": {
        "isbn10": "${book_data[_g_isbn10]:-}",
        "isbn13": "${book_data[_g_isbn13]:-}",
        "oclc": "",
        "lccn": "",
        "other": ""
    },
    ...
    "physical": {
        "pages": "$pages",
        "binding": "$binding",
        "printType": "${book_data[_g_printType]:-}",
    ...
    "classification": {
        ...
        "vintedCategoryId": "$vinted_id",
        "vintedCategoryName": "$vinted_name"
    },
    "commercial": {
        "msrp": "${price:-}",
        "retailPrice": "",
    ...
    "ratings": {
        "average": "${book_data[_g_averageRating]:-}",
        "count": "${book_data[_g_ratingsCount]:-}",
        "maturity": ""
    },
    "content": {
        "description": "$(echo "$description" | sed 's/"/\\"/g' | tr '\n' ' ')",
        "notes": "",
        "searchTerms": "$(echo "$title $authors $publisher $categories" | tr '\n' ' ')",
        "bulletPoints": [
            "$(echo "${book_data[_calculated_bullet1]:-}" | sed 's/"/\\"/g')",
            "$(echo "${book_data[_calculated_bullet2]:-}" | sed 's/"/\\"/g')",
            "$(echo "${book_data[_calculated_bullet3]:-}" | sed 's/"/\\"/g')",
            "$(echo "${book_data[_calculated_bullet4]:-}" | sed 's/"/\\"/g')",
            "$(echo "${book_data[_calculated_bullet5]:-}" | sed 's/"/\\"/g')"
        ]
    },
    "media": {
        "image": "$best_image",
        "previewLink": "${book_data[_g_previewLink]:-}",
        "infoLink": "${book_data[_g_infoLink]:-}"
    }
}
EOF

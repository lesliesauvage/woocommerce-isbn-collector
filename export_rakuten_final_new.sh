...
analyze_csv() {
   ...
   # Les caractères é, è, ç sont AUTORISÉS en français
   # On vérifie seulement les vrais caractères Microsoft interdits
   
   # Utiliser grep -F pour éviter les problèmes avec les caractères spéciaux
   if echo "$data_only" | grep -F "'" > /dev/null 2>&1; then
       echo "❌ Apostrophe courbe gauche (') trouvée"
       ((bad_chars++))
   fi
   
   if echo "$data_only" | grep -F "'" > /dev/null 2>&1; then
       echo "❌ Apostrophe courbe droite (') trouvée"
       ((bad_chars++))
   fi
   
   if echo "$data_only" | grep -F """ > /dev/null 2>&1; then
       echo "❌ Guillemet courbe gauche (") trouvé"
       ((bad_chars++))
   fi
   
   if echo "$data_only" | grep -F """ > /dev/null 2>&1; then
       echo "❌ Guillemet courbe droit (") trouvé"
       ((bad_chars++))
   fi
   
   if echo "$data_only" | grep -F "…" > /dev/null 2>&1; then
       echo "❌ Points de suspension (…) trouvés"
       ((bad_chars++))
   fi
   
   if echo "$data_only" | grep -F "—" > /dev/null 2>&1; then
       echo "❌ Tiret cadratin (—) trouvé"
       ((bad_chars++))
   fi
   
   if echo "$data_only" | grep -F "–" > /dev/null 2>&1; then
       echo "❌ Tiret demi-cadratin (–) trouvé"
       ((bad_chars++))
   fi
   
   if echo "$data_only" | grep -F "«" > /dev/null 2>&1; then
       echo "❌ Guillemet français ouvrant («) trouvé"
       ((bad_chars++))
   fi
   
   if echo "$data_only" | grep -F "»" > /dev/null 2>&1; then
       echo "❌ Guillemet français fermant (») trouvé"
       ((bad_chars++))
   fi
   
   if [ $bad_chars -eq 0 ]; then
       echo "✅ Aucun caractère Microsoft interdit dans les données"
   else
       ((errors++))
   fi

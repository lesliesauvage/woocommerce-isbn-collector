...
collect_book_data() {
    ...
    store_meta "$product_id" "_api_collect_status" "processing"
    ...
    if [ "$final_pages" -gt 0 ]; then
        ...
        store_meta "$product_id" "_calculated_weight" "$calculated_weight"
        log "      → Poids calculé : ${calculated_weight}g"
    fi
    
    # Calculer les dimensions
    local dimensions=$(calculate_dimensions "$final_binding")
    store_meta "$product_id" "_calculated_dimensions" "$dimensions"
    ...
    store_meta "$product_id" "_api_collect_date" "$(date '+%Y-%m-%d %H:%M:%S')"
    store_meta "$product_id" "_api_collect_status" "completed"
    store_meta "$product_id" "_api_collect_version" "v2_with_counters"
    ...
    store_meta "$product_id" "_api_calls_made" "$total_calls"
...
while IFS=$'\t' read -r product_id isbn price; do
    ...
    # Gérer les erreurs par livre
    {
        ...
        store_meta "$product_id" "_api_collect_status" "error"
